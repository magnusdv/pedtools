...
# Pedigrees
In `pedtools` and all related packages, pedigrees are stored as `ped` objects. We start by explaining briefly what these objects look like, and their basic constructor. If you are reading this vignette simply to learn how to create a particular pedigree, you may want to skip ahead to section 1.3 where we describe pratical shortcuts to common pedigree structures.
...
* `sex` : gender codes, with entries 0 (unkown), 1 (male) or 2 (female)
...
Several options are available for controlling the appearence of individual pedigree members and the plot as a whole. A complete overview can be found in the documentation `?plot.ped`, but a quick example should get you started:
...
* `cousinsPed()`, cousins of specified degree/removal
* `halfCousinsPed()`, half cousins of specified degree/removal
...
**Alternative B**  
We could also view the half siblings 4 and 5 as half cousins of degree 0. The `halfCousinsPed()` function accepts an option `child = TRUE` adding an inbred child. The labels will be different with this approach, so you should plot the pedigree after each command to see who-is-who. Also, we must relabel in the end.

```{r}
x2 = halfCousinsPed(0, child = T)
...
Allthough both `x1` and `x2` reproduce exactly the plot shown above, they are not identical objects: 
```{r}
identical(x1, x2)
```
...
[^2]: There is an important exception to this: Certain algoritms in pedigree analysis work "top-down", in the sense that parents must be treated before their children. For this reason, many implementations require, for simplicity, that the individuals are stored in this fashion, i.e. that parents always precede their children. `pedtools` offers a special reordering function to ensure this, `parents_before_children()`, which you will find utilised in the source code of packages like `ribd` and `ibdsim2`.
...
When creating the parts that are to be merged, it is usually best to keep the overlap minimal. In our code below we chose the mother to be the single point of contact. To understand the relabeling, plot `y1` and `y2` after each command.
...
The appearence of the genotypes can be tweaked in various ways, as documented in `?plot.ped`. Here's an example:
```{r, eval=FALSE}
